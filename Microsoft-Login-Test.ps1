...
if ($deviceResp.verification_uri) { Start-Process $deviceResp.verification_uri }
...
while ($true) {
    ...
    try {
        ...
        if ($resp -ne $null) {
            ...
            if ($err -and $err.error -eq 'expired_token') { throw "Device code expired. Restart process." }
            if ($err -and $err.error -eq 'access_denied') { throw "User denied the request." }
            throw $content
        } else {
            throw $_
        }
